...
public partial class RainMeadow
{
    ...
    private void Player_ctor(On.Player.orig_ctor orig, Player self, AbstractCreature abstractCreature, World world)
    {
        ...
        if (OnlineManager.lobby != null)
        {
            if (OnlinePhysicalObject.map.TryGetValue(self.abstractPhysicalObject, out var ent))
            {
                ...
                RainMeadow.Debug($"slugcatstats{self.SlugCatClass} owner:{ent.owner}");
            }
            else
            {
    ...
    private SlugcatStats Player_slugcatStats(Func<Player, SlugcatStats> orig, Player self)
    {
        if (OnlineManager.lobby != null)
        {
            ...
            {
                OnlinePhysicalObject.map.TryGetValue(self.abstractPhysicalObject, out var ent);
                RainMeadow.Debug($"slugcatstats{slugcatStats.name} owner:{ent.owner}");
                return slugcatStats;
            }
        }
